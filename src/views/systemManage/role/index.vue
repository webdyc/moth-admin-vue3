  ...
  >
    <div class="table-modal-content" v-ant-drag-dialog>
      ...
      <div class="right">
        <h3>菜单配置</h3>
        <a-tree
          v-model:checkedKeys="updateForm.menuIdList"
          checkable
          :tree-data="menuTree"
          :field-names="fieldNames"
          @check="handleMenuTree"
        >
...
export default {
  ...
  setup() {
    ...
    const getMenuTree = async () => {
      let { code, data } = await menu_treeselectrole()
      if (code === 200) {
        stateData.menuTree = data
      }
    }
    // 组织树选择
    ...
    const handleSearch = async () => {
      ...
      // 获取角色列表
      let { code, rows, total } = await role_list(stateData.searchForm)
      if (code === 200) {
    ...
    const handleEdit = (state, row) => {
      ...
      if (stateData.isUpdate) {
        ...
        console.log(stateData.updateForm)
    ...
    const handleSubmit = debounce(async () => {
      ...
      if (!stateData.isUpdate) {
        let { code } = await role_add(stateData.updateForm)
        if (code === 200) {
        ...
        let { code } = await role_edit(stateData.updateForm)
        if (code === 200) {
    ...
    const handelSwitchChange = (row) => {
      Modal.confirm({
        ...
        cancelText: '取消',
        async onOk() {
          let { code } = await role_stateChange({
            id: row.id,
